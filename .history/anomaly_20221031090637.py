...
# isolation forest
def forest_preprocess(df_CTR):
    countries = df_CTR["LCB "].drop_duplicates().to_list()

    list_df_anom = list()
    list_df_no_anom = list()

    for country in countries:
        df_CTR_country = df_CTR.loc[df_CTR["LCB "] == country]
        df_CTR_country_anom = df_CTR_country.loc[:, ["Date", "Value", "Label"]].set_index("Date")
        df_CTR_country_no_anom = df_CTR_country_anom.loc[((df_CTR_country_anom["Label"] != "low peak") & (df_CTR_country_anom["Label"] != "high peak" ) & (df_CTR_country_anom["Label"] != "very low & negative period" ) & (df_CTR_country_anom["Label"] != "very low period" ))]
        
        list_df_anom.append(df_CTR_country_anom)
        list_df_no_anom.append(df_CTR_country_no_anom)

    return list_df_anom

def predict_forest(filename, df):
    data = df
    ...
    # TODO fix overlay of anomalies
    fig5 = px.plot(df.index, df['Value'], color='black', label = 'Normal')
    fig4 = px.scatter(a.index,a['Value'], color='red', label = 'Anomaly')
    fig4.xlabel("Date")
    fig4.ylabel("Value")
    fig4.legend()
